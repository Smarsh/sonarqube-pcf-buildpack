...
vcap_username=`echo $VCAP_SERVICES | jq -r '.["p.mysql"][]["credentials"]["username"]'`
vcap_password=`echo $VCAP_SERVICES | jq -r '.["p.mysql"][]["credentials"]["password"]'`
vcap_jdbc_url=`echo $VCAP_SERVICES | jq -r '.["p.mysql"][]["credentials"]["jdbcUrl"]'`
